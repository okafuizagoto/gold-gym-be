...
func HTTP() error {
	...
	// Open MySQL DB Connection
	dbprod, err := openDatabasesProd(cfg)
	if err != nil {
		log.Fatalf("[DB] Failed to initialize database connection: %v", err)
	}
	...
	viper.OnConfigChange(func(e fsnotify.Event) {
		...
		// reload prod db
		masterNewProduction, err := openDatabasesProd(cfg)
		if err != nil {
...
func openDatabasesProd(cfg *config.Config) (master *sqlx.DB, err error) {
	master, err = openConnectionPool("mysql", cfg.Database.Production)
	if err != nil {
		return master, err
	}
